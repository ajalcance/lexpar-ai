...
def stream_reply(state: SessionState, attorney_turn: str) -> Iterator[str]:
    """Stream Opposing Counsel's next reply as text deltas. Makes a live API call."""
    endpoint = build_endpoint(opposing_counsel_config())
    yield from chat_stream(
        endpoint, build_messages(state, attorney_turn), temperature=0.7, max_tokens=400
    )
