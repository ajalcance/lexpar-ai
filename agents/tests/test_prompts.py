...
STATIC_GOLDENS = {
    ...
    "judge_quick_ruling": (
        ...
        "overrule. FIRST CHECK: if the statement merely identifies the case — its docket or case "
        "number, its caption or parties, its procedural posture, or what relief is sought — that "
        "is routine housekeeping, not a factual assertion, EVEN AT THE OPENING WHEN THE RECORD IS "
        "STILL EMPTY: OVERRULE any objection to it before weighing anything else.\n"
        "\n"
...
def test_classifier_prompt_renders_byte_identical_with_eligible_grounds():
    ...
    expected = (
        ...
        "grounds already ruled. NEVER fire on routine case identification or procedural "
        "housekeeping — the case's number or caption, the parties, the proceeding, or the relief "
        "sought — even at the opening when the record is still empty (an empty record does not "
        "make the case's own background an unestablished fact).\n"
        "\n"
        ...
        "so hold the bar high. For relevance and mischaracterizes_record, make the comparison "
